This page introduces the Cyberismo query language. A Cyberismo query uses logic programming facts and rules to "sculpt" its result as a JavaScript Object Notation (JSON) object.
...
* Scalar properties, described with the `field` predicate. Fields can come from the global logic program, or they can be fields that are introduced in the query
