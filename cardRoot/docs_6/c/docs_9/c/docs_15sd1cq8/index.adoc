...
If the external system of a connector supports linking or associating external items with each other, or if there are other relationships between external items, then the connector should generate `externalLink` facts facts about these links, see `{{#xref}}"cardKey":"docs_5ts51i9i"{{/xref}}`.
