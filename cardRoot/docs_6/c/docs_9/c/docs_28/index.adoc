...
== Conditional literals or nested rules

Sometimes you need to express a condition that must hold for all occurrences of a certain set, or none of the occurrences of a certain set. This can be done with a so called conditional litteral, where a body condition of a rule consists of a nested rule of the form 

_nested head_ `:` __condition 1__``,`` __condition 2__``,`` ...``,`` _condition n_
...
In this rule, `b` and `e` are normal positive body conditions and `c : d` is a nested rule, or a conditional literal. If `b` and `e` are true, then this rule will yield `a` if also the nested rule `c : d` holds. The nested rule holds, if `c` holds whenever `d` holds, in other words when either both `c` and `d` are true, or whenever `d` is false. 
