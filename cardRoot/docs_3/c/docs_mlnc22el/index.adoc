...
==== Adding default hub
If user has removed the default hub, or the project was created before hub support was added to the Cyberismo tool, the default hub can be added to the project with command:
...
You can also remove the default hub, if you don't think you needed with command:
[source,console]
----
...
NOTE: When creating a new project, it will automatically prompt for importing Cyberismo company defined modules.
...
NOTE: The modules that were imported from the local file system must reside in the same directory where they were imported from.
